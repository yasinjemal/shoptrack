/**
 * English strings -- the source of truth for the Strings shape.
 *
 * Every other language file is typed as `typeof en`, so a key missing there
 * is a compile error caught by `npm run typecheck`, not a blank label in a shop.
 * Extracted verbatim from App.tsx (the inline STRINGS dictionary).
 */

import type { DueOptionKey, PaymentMethod } from '../core/credit';
import type { ExpenseCategory } from '../core/expenses';
import { formatShopDateTime } from '../core/localDate';

export const en = {
    // Count screen
    COUNT_HEADER: "How many do you have right now?",
    COUNT_HINT: "Count what's on your shelves",
    COUNT_REVIEW_TITLE: "Review your count",
    COUNT_REVIEW_HINT: "Check these numbers before they change your profit.",
    COUNT_FIRST_VALUE: (name: string, qty: number) => `${name}: first count ${qty}`,
    COUNT_CHANGE_VALUE: (name: string, before: number, after: number) => `${name}: ${before} → ${after}`,
    COUNT_REVIEW_BUTTON: (count: number) => `Review ${count} counted`,
    COUNT_SAVE_BUTTON: "Save count",
    COUNT_GO_BACK: "Go back",
    COUNT_UNDO: "Undo this count",
    COUNT_UNDO_CONFIRM: "This removes the latest count and restores the stock quantities from before it.",
    COUNT_UNDONE: "Count removed",
    COUNT_UNDONE_HINT: "Your previous stock quantities are back.",
    COUNT_UNDO_EXPIRED: "This count can no longer be undone.",
    COUNT_REMINDER_TITLE: "Time to count again",
    COUNT_REMINDER_HINT: "It has been a week. Count what's left to refresh your profit.",
    FIRST_COUNT_HINT: "This is your starting point — we'll compare future counts to this.",
    
    // Results - Profit Explanation (Tier 3.1)
    SOLD_SINCE: (n: number) => `You sold ${n} item${n !== 1 ? 's' : ''} since your last count.`,
    PROFIT_MADE: (amount: string) => `You made ${amount} profit`,
    NO_CHANGE: "No sales since your last count.",
    STOCK_INCREASED: "You added more stock than you sold.",
    UNUSUAL_CHANGE: "This change looks unusual. You may want to recount.",
    
    // Weekly
    WEEKLY_HEADING: "This Week",
    WEEKLY_YOU_MADE: "You made:",
    WEEKLY_PROFIT: (amount: string) => `${amount} profit`,
    WEEKLY_TOP_PRODUCT: "Top product:",
    WEEKLY_COMPARED: "Compared to last week:",
    WEEKLY_MORE: (amount: string) => `↑ ${amount} more`,
    WEEKLY_LESS: (amount: string) => `↓ ${amount} less`,
    WEEKLY_FIRST_WEEK: "This is your first week tracking.",
    WEEKLY_FIRST_WEEK_HINT: "Count your stock a few times and your profit will show here.",
    WEEKLY_NO_COUNTS: "No counts yet this week",
    WEEKLY_NO_COUNTS_HINT: "Count your stock to see this week's profit.",
    WEEKLY_KEEP_COUNTING: "Keep counting! Next week you'll see how you compare.",
    
    // Backup
    BACKUP_SAVED: "Saved!",
    BACKUP_HINT: "Your data is safe. Keep this file somewhere you won't lose it — like WhatsApp or Google Drive.",
    BACKUP_CUSTOMER_PHOTOS_EXCLUDED: "For privacy, this shared file leaves out customer/ID photos. Encrypted backups keep them.",
    BACKUP_NUDGE_TITLE: "Keep a copy outside this phone",
    BACKUP_NUDGE_HINT: "Your last shared backup is over a week old. Tap to send a fresh one.",
    CRASH_NOTICE_TITLE: "ShopTrack closed unexpectedly last time",
    CRASH_NOTICE_HINT: "The details were saved on this phone and will be included in your next backup.",
    RESTORE_DONE: "Done!",
    RESTORE_DONE_HINT: "Your shop data is back.",
    RESTORE_INVALID: "Not a ShopTrack backup",
    RESTORE_INVALID_HINT: "This doesn't look like a ShopTrack backup. Try a different file.",
    RESTORE_OLD_VERSION: "Backup is too old",
    RESTORE_OLD_VERSION_HINT: "This backup was made by an older version of ShopTrack and can't be opened. Use a newer backup.",
    RESTORE_CONFIRM: "Restore backup?",
    RESTORE_CONFIRM_HINT: "This will replace all your current data with the backup. Are you sure?",
    
    // Errors
    ERROR_GENERIC: "Something went wrong",
    ERROR_TITLE: "Error",
    ERROR_BACKUP: "Could not create backup",
    ERROR_RESTORE: "Could not restore backup",
    DB_ERROR_TITLE: "Can't open your shop data",
    DB_ERROR_HINT: "Your data is still on this phone — ShopTrack just couldn't read it. Try again, or close the app and open it fresh.",
    DB_ERROR_RETRY: "Try again",
    
    // Confidence Signals (Tier 3.2)
    BASED_ON_COUNTS: (n: number) => `Based on ${n} count${n !== 1 ? 's' : ''}`,
    CONFIDENCE_EARLY: "Early estimate",
    CONFIDENCE_CLEARER: "Getting clearer",
    CONFIDENCE_RELIABLE: "Reliable",
    MISSING_STOCKIN: "This profit doesn't include purchases you didn't record.",
    
    // Stock Value (Tier 3.3)
    STOCK_VALUE_LABEL: "Stock on hand (at cost)",
    STOCK_VALUE_HINT: "Based on purchase prices",
    STOCK_VALUE_MISSING_PRICES: "Some products missing buy prices",
    
    // Restock Priority (Tier 4.1)
    RESTOCK_TITLE: "Restock priority",
    RESTOCK_HINT: "Based on recent sales and stock levels",
    RESTOCK_FAST_LOW: "sells fast, low stock",
    RESTOCK_FAST: "sells fast",
    RESTOCK_PROFIT: "high profit",
    
    // Money Tied Up (Tier 4.2)
    SLOW_STOCK_TITLE: "Money tied up in slow stock",
    SLOW_STOCK_HINT: "Stock with little or no movement this week",
    SLOW_STOCK_LABEL: "slow",
    
    // Silent Loss Detector (Tier 4.3)
    LOSS_TITLE: "Some products are losing profit",
    LOSS_HINT: "Selling below purchase price",
    
    // Owner Memory (Tier 4.4)
    OWNER_MEMORY_TITLE: "This week you mostly sold",

    // Credit book / izikweletu
    CREDIT_TITLE: "Credit Book",
    CREDIT_HOME_BUTTON: "Credit Book",
    CREDIT_HOME_HINT: "Who owes you",
    CREDIT_EMPTY: "No credit yet",
    CREDIT_EMPTY_HINT: "When someone takes goods and pays later, write it here so you don't forget.",
    CREDIT_ALL_PAID: "Everyone is paid up",
    CREDIT_ALL_PAID_HINT: "Nobody owes you anything right now.",
    CREDIT_OUTSTANDING_LABEL: "Owed to you",
    CREDIT_WEEK_SUMMARY: (given: string, paid: string) => `This week: ${given} given, ${paid} paid back`,
    CREDIT_STALE_TITLE: "Not paid for a long time",
    CREDIT_STALE_HINT: "These people may need a reminder.",
    CREDIT_OVERDUE_TITLE: "Said they would pay by now",
    CREDIT_OVERDUE_HINT: "They named a day and it has passed.",
    CREDIT_ADD_CUSTOMER: "Add Person",
    CREDIT_MANAGE_CUSTOMER: "Manage Person",
    CREDIT_REMOVE_CUSTOMER: "Remove Person",
    CREDIT_REMOVE_CUSTOMER_CONFIRM: (name: string) => `Remove ${name} from the credit book? Their history will be kept.`,
    CREDIT_CUSTOMER_NAME: "Name",
    CREDIT_CUSTOMER_PHONE: "Phone",
    CREDIT_PHONE_OPTIONAL: "Optional — leave empty if you don't have it.",
    CREDIT_TAKING_NOW: "How much are they taking?",
    CREDIT_TAKING_HINT: "Leave empty if they're not taking anything yet.",
    CREDIT_WHEN_PAY: "When will they pay?",
    CREDIT_DUE_OPTION: (key: DueOptionKey) => ({
      friday: 'Friday',
      end_of_month: 'End of month',
      two_weeks: 'In 2 weeks',
      unknown: "Didn't say",
    })[key],
    CREDIT_GIVE: "Gave credit",
    CREDIT_RECEIVE: "Got paid",
    CREDIT_AMOUNT: "How much?",
    CREDIT_NOTE: "What did they take?",
    CREDIT_NOTE_HINT: "Bread and milk",
    CREDIT_SAVE: "Save",
    CREDIT_SAVING: "Saving...",
    CREDIT_DAYS_QUIET: (days: number) => days === 0 ? "today" : days === 1 ? "1 day ago" : `${days} days ago`,
    CREDIT_DUE_IN: (days: number) => days === 0 ? "Says they'll pay today" : days === 1 ? "Says they'll pay tomorrow" : `Says they'll pay in ${days} days`,
    CREDIT_OVERDUE_BY: (days: number) => days === 0 ? "was due today" : days === 1 ? "1 day late" : `${days} days late`,
    CREDIT_PAID_UP: "They will be all paid up",
    CREDIT_PAID_UP_TAG: "Paid up",
    CREDIT_OWES_YOU_CHANGE: "You owe them change",
    CREDIT_CURRENT_OWES: (amount: string) => `Owes you ${amount}`,
    CREDIT_CURRENT_CHANGE: (amount: string) => `You owe them ${amount} change`,
    CREDIT_PAYMENT_METHOD: "How did they pay?",
    CREDIT_PAYMENT_METHOD_LABEL: (method: PaymentMethod) => ({
      CASH: 'Cash', MOBILE_MONEY: 'Mobile money', BANK: 'Bank transfer', OTHER: 'Other',
    })[method],
    CREDIT_PROJECTED_OWES: (name: string, amount: string) => `${name} will owe ${amount}`,
    CREDIT_PROJECTED_CHANGE: (amount: string) => `You will owe ${amount} in change`,
    // Shown next to profit, never subtracted from it (see src/core/credit.ts)
    CREDIT_NOT_IN_HAND: (amount: string) => `${amount} of this is still owed to you.`,

    // Expenses
    EXPENSES_TITLE: "Expenses",
    EXPENSES_HOME_BUTTON: "Expenses",
    EXPENSES_HOME_HINT: "What you pay out",
    EXPENSES_EMPTY: "No expenses yet",
    EXPENSES_EMPTY_HINT: "Rent, electricity, transport, wages. Add them here so your profit is the real number.",
    EXPENSES_MONTH_LABEL: "Paid out this month",
    EXPENSES_ADD: "Add Expense",
    EXPENSES_AMOUNT: "How much?",
    EXPENSES_CATEGORY: "What was it for?",
    EXPENSES_NOTE: "Note",
    EXPENSES_NOTE_HINT: "Taxi to cash and carry",
    EXPENSES_SAVE: "Save",
    EXPENSES_SAVING: "Saving...",
    EXPENSES_NOT_STOCK: "Don't add stock you bought here — use Add Stock for that, or it gets counted twice.",
    EXPENSES_DELETE_CONFIRM: "Remove this expense?",
    EXPENSES_DELETE_CONFIRM_HINT: "It will be taken off your totals.",
    EXPENSES_DELETE: "Remove",
    EXPENSES_CANCEL: "Cancel",
    CATEGORY_LABEL: (c: ExpenseCategory) => ({
      RENT: 'Rent',
      ELECTRICITY: 'Electricity',
      TRANSPORT: 'Transport',
      WAGES: 'Wages',
      AIRTIME: 'Airtime & data',
      OTHER: 'Other',
    })[c],

    // Net profit (gross - expenses)
    NET_FROM_SALES: "From sales:",
    NET_EXPENSES: "Costs:",
    NET_KEPT: "You kept:",
    NET_LOSS: "You are short:",
    NET_NO_EXPENSES: "No expenses recorded, so this is before rent and costs.",

    // Cash up
    CASHUP_TITLE: "Cash Up",
    CASHUP_HOME_BUTTON: "Cash Up",
    CASHUP_HOME_HINT: "Count your till",

    // Sales book
    SALES_TITLE: "Sales Book",
    SALES_HOME_BUTTON: "Sales Book",
    SALES_HOME_HINT: "What you took",
    SALES_EMPTY: "No sales written down yet",
    SALES_EMPTY_HINT: "Keep a book of what you take each day? Write it here and ShopTrack works out what you made — even for months before you got the app.",
    SALES_TOTAL_LABEL: "Profit so far",
    SALES_TOTAL_HINT: (months: number, margin: string) =>
      `From ${months} ${months === 1 ? 'month' : 'months'}, keeping about ${margin}`,
    SALES_TODAY: "Today's sales",
    SALES_BACKFILL: "Add a past month",
    SALES_TOOK_TODAY: "How much did you take today?",
    SALES_DAYS_FILLED: (filled: number, total: number) => `${filled} of ${total} days filled in`,
    SALES_FILL_HINT: "Fill in the days you know. Leave the rest empty. Put 0 for a day you were closed.",
    SALES_MARGIN: "How much of that do you keep?",
    SALES_MARGIN_HINT: (hundred: string, low: string, high: string) => `Out of every ${hundred} you take, how much is yours after paying for the stock? Most shops keep ${low}–${high}.`,
    SALES_MARGIN_IS_YOURS: "This is your own estimate, so the profit is an estimate too. Count your stock for the exact number.",
    SALES_WILL_KEEP: (amount: string) => `You kept about ${amount}`,
    SALES_PICK_MONTH: "Which month?",
    SALES_SAVE: "Save",
    SALES_SAVING: "Saving...",
    SALES_CANCEL: "Cancel",
    SALES_MONTH_DAYS: (days: number) => `${days} ${days === 1 ? 'day' : 'days'} written down`,
    SALES_MONTH_TOTAL: "whole month",
    SALES_CONFLICT: "One month is written down twice",
    SALES_CONFLICT_FIX: "tap to keep the daily entries",
    SALES_NOT_COUNTED_PROFIT: "This is from your own book. Counting stock gives its own profit — the two answer the same question in different ways, so don't add them together.",
    SALES_STATS_TITLE: "Sales patterns",
    SALES_STATS_FOR: (month: string) => `Latest recorded month: ${month}`,
    SALES_STATS_BEST_DAY: "Highest takings day",
    SALES_STATS_QUIETEST_DAY: "Lowest takings day",
    SALES_STATS_NO_DAILY: "Add individual days to see the highest and lowest day. A whole-month total cannot show this.",
    SALES_STATS_MONTH_CHANGE: "Month to month",
    SALES_STATS_VS_MONTH: (change: string, month: string) => `${change} compared with ${month}`,
    SALES_STATS_NO_PREVIOUS_MONTH: "No immediately previous month is recorded, so there is no fair comparison.",
    SALES_STATS_YTD: "Year to date takings",
    SALES_STATS_YTD_COVERAGE: (recorded: number, elapsed: number, month: string) =>
      `${recorded} of ${elapsed} months recorded through ${month}`,
    SALES_STATS_VS_YEAR: (change: string, year: number, recorded: number) =>
      `${change} compared with ${year} (${recorded} months recorded)`,
    SALES_STATS_NO_PREVIOUS_YEAR: "No matching months are recorded for the previous year.",
    CASHUP_FIRST_TITLE: "Starting Cash",
    CASHUP_FIRST_HINT: "This is your starting point. Next time you cash up, we'll compare against this.",
    CASHUP_QUESTION: "How much cash is in your till?",
    CASHUP_HINT: "Count it before you look at anything else.",
    CASHUP_SINCE: (when: string) => `Since your last cash up: ${when}`,
    CASHUP_CHECK: "Check",
    CASHUP_CHECKING: "Saving...",
    CASHUP_SAVE: "Save",
    CASHUP_TRAIL_TITLE: "How we worked it out",
    CASHUP_LINE_OPENING: "Was in the till",
    CASHUP_LINE_REVENUE: "Sold",
    CASHUP_LINE_DIGITAL: "Sales paid into phone / bank",
    CASHUP_LINE_CREDIT: "Given on credit",
    CASHUP_LINE_PAYMENTS: "Debts paid to you",
    CASHUP_LINE_EXPENSES: "Expenses paid",
    CASHUP_LINE_STOCK: "Stock bought",
    CASHUP_EXPECTED: "Should be there",
    CASHUP_COUNTED: "You counted",
    CASHUP_BALANCED: "Your till is right",
    CASHUP_SHORT: "Money is missing",
    CASHUP_OVER: "There is extra",
    CASHUP_TAKE_OUT: "Taking money out?",
    CASHUP_TAKE_OUT_HINT: "If you're taking cash out now, put it here so tomorrow's count starts right.",
    CASHUP_NO_COUNT_WARNING: "You haven't counted stock since your last cash up, so the 'sold' number is a guess. Count your stock first for a real answer.",
    CASHUP_DONE: "Done",
    CASHUP_HISTORY: "Last few cash ups",
    CASHUP_CANCEL: "Cancel",
    CASHUP_HOME_SHORTFALL: "was missing at your last cash up. Tap to look again.",
    CASHUP_FLOAT: "float",
    CASHUP_DIGITAL_QUESTION: "How much of the sales went to your phone or bank?",
    CASHUP_DIGITAL_HINT: "Mobile money, card, and transfer sales only. Debt repayments are already split in the credit book.",
    CASHUP_STATEMENT_BALANCED: "Your till matches what the app expected.",
    CASHUP_STATEMENT_OVER: (amount: string) => `You have ${amount} more than expected. A sale or payment may not have been written down.`,
    CASHUP_STATEMENT_SHORT_LARGE: (amount: string) => `You are ${amount} short. Check stock bought, an expense paid, or credit given that was not recorded.`,
    CASHUP_STATEMENT_SHORT_SMALL: (amount: string) => `You are ${amount} short. This is often change given or a small expense not written down.`,
    FORMAT_WHEN: (ts: number) => formatShopDateTime(ts, 'en-ZA'),

    // Shared app, product, and activity UI
    APP_TAGLINE: "Know your profit",
    STARTING_APP: "Starting ShopTrack...",
    HOW_TITLE: "📊 How it works",
    HOW_STEP_1: "Add your products (name + prices)",
    HOW_STEP_2: "Count your stock today",
    HOW_STEP_3: "Count again in a few days",
    HOW_STEP_4: "See your profit here! ⬇️",
    HOW_NOTE: "Count what's left and ShopTrack works out what sold and what you made.",
    YOUR_PROFIT: "Your Profit",
    PROFIT_EXPLAINER: "Based on stock sold since your last count",
    PRODUCTS_LABEL: "Products",
    ITEMS_IN_STOCK: "Items in Stock",
    COUNT_STOCK: "Count Stock",
    COUNT_TO_PROFIT: "Count now to refresh your profit",
    COUNT_BASELINE: "First count = your starting point",
    ADD_STOCK: "Add Stock",
    WHEN_YOU_BUY: "When you buy",
    ADD_OR_EDIT: "Add or edit",
    RECENT_ACTIVITY: "Recent Activity",
    THIS_WEEK: "This Week",
    RUNNING_LOW: "⚠️ Running Low",
    STOCK_LEFT: (name: string, qty: number) => `${name}: ${qty} left`,
    WELCOME_TITLE: "👋 Welcome to ShopTrack!",
    WELCOME_TEXT: "Add what you sell. We'll show you how much you make.",
    WELCOME_HOW: "Start with three products. Count what's left regularly and ShopTrack works out what sold.",
    ADD_FIRST_PRODUCT: "Add Your First Product",
    YOUR_DATA: "Your Data",
    SAVE_DATA: "Save a copy of your shop data",
    SAVE_DATA_HINT: "Keep it safe on WhatsApp or Google Drive",
    RESTORE_DATA: "Restore from backup",
    RESTORE_DATA_HINT: "Get all your data back on a new phone",
    BACKUP_DIALOG_TITLE: "Save your ShopTrack backup",
    RESTORE_ACTION: "Restore",
    BACK: "← Back",
    CANCEL: "Cancel",
    ADD: "+ Add",
    SEARCH_PRODUCTS: "Search products...",
    NO_PRODUCTS: "No products yet",
    ADD_PRODUCT: "Add Product",
    READY_TO_TRACK: "You're ready to track",
    READY_TO_TRACK_HINT: "Add another product, or start counting when you're ready.",
    START_COUNTING: "Start counting",
    ADD_PRODUCTS_FIRST: "Add products first",
    NO_PRODUCT_MATCH: (query: string) => `No products match "${query}"`,
    PRODUCT_META: (qty: number, unit: string, sell: string | null, buy: string | null) =>
      `${qty} ${unit}${sell != null ? ` • Sell: ${sell}` : ''}${buy != null ? ` • Buy: ${buy}` : ''}`,
    CREDIT_HOME_STATUS: (owing: number, overdue: number, stale: number) =>
      `${owing === 1 ? '1 person' : `${owing} people`}${overdue > 0 ? ` · ${overdue} late` : stale > 0 ? ` · ${stale} not paid in a while` : ''}`,
    WHAT_DO_YOU_SELL: "What do you sell?",
    PRODUCT_EXAMPLE: "e.g., Coca-Cola 500ml",
    SELL_PRICE_OPTIONAL: "Customer pays (optional)",
    BUY_PRICE_OPTIONAL: "You pay (optional)",
    CUSTOMER_PAYS: "What you charge customers",
    YOU_PAY: "What you pay for it",
    CURRENT_STOCK_OPTIONAL: "How many now? (optional)",
    HOW_MANY_NOW: "How many do you have right now?",
    SAVING: "Saving...",
    EDIT_PRODUCT: "Edit Product",
    SELL_PRICE: "Customer pays",
    BUY_PRICE: "You pay",
    CURRENT_STOCK: (qty: number, unit: string) => `Current stock: ${qty} ${unit}`,
    USE_COUNT_TO_UPDATE: "Use Count Stock to update quantity",
    SAVE_CHANGES: "Save Changes",
    DELETE_PRODUCT: "Delete Product",
    DELETE_PRODUCT_CONFIRM: (name: string) => `Delete "${name}"? Its history will be kept, but it will no longer appear in your lists.`,
    FIRST_COUNT_DONE: "First Count Done!",
    COUNT_SAVED: "Count Saved!",
    STARTING_STOCK_RECORDED: "You've recorded your starting stock.",
    GOT_IT: "Got it!",
    DONE: "Done",
    COUNT_PROGRESS: (done: number, total: number) => `${done} of ${total} counted`,
    NOT_COUNTED_YET: "Not counted yet",
    LAST_COUNT: (qty: number) => `Last: ${qty}`,
    ERROR_SAVE_PRODUCT: "Could not save product",
    ERROR_UPDATE_PRODUCT: "Could not update product",
    ERROR_DELETE_PRODUCT: "Could not delete product",
    ERROR_SAVE_COUNT: "Could not save count",
    SHARING_UNAVAILABLE: "Sharing is not available on this device",
    STOCK_ADDED: "Stock Added! ✓",
    STOCK_ADDED_HINT: (qty: number, unit: string, name: string) => `${qty} ${unit} of ${name} recorded.`,
    UNDO: "Undo",
    WHAT_DID_YOU_BUY: "What did you buy?",
    IN_STOCK: (qty: number, unit: string) => `${qty} ${unit} in stock`,
    CURRENTLY_IN_STOCK: (qty: number, unit: string) => `Currently: ${qty} ${unit}`,
    HOW_MANY_BOUGHT: "How many did you buy?",
    TOTAL_COST: "Total cost?",
    COST_MODE_TOTAL: "Total cost",
    COST_MODE_EACH: "Per item",
    COST_PER_ITEM: "Cost per item?",
    COST_EACH: (amount: string) => `= ${amount} each`,
    COST_TOTAL: (qty: number, each: string, total: string) => `${qty} × ${each} = ${total}`,
    SAVE_STOCK_IN: "Save Stock-In",
    ERROR_SAVE_STOCK: "Could not save stock",
    ERROR_UNDO_STOCK: "Could not undo stock-in",
    WEEKLY_SUMMARY: "Weekly Summary",
    TOP_SELLERS: "🏆 Top Sellers",
    SINCE_LAST_COUNT: "Since last count",
    PRODUCT_DETAILS: "Product Details",
    CURRENT_STOCK_ACTIVITY: "Current stock:",
    SOLD_SINCE_LAST: "Sold since last count:",
    PROFIT_LABEL: "Profit:",
    LAST_COUNTS: (counts: number[]) => `Last counts: ${counts.join(' → ')}`,
    NO_ACTIVITY: "No activity yet",

    // Language
    LANGUAGE_LABEL: "Language",

    // Structured engine statement renderers
    STAT_TRUTH_NO_DATA: (name: string) => `Not enough data to estimate ${name} sales.`,
    STAT_TRUTH_IMPOSSIBLE_GAIN: (name: string) => `${name} shows more stock than recorded. Check for a missed delivery.`,
    STAT_TRUTH_LOSS: (name: string) => `${name} may have loss or damage. Stock is lower than expected.`,
    STAT_TRUTH_INCORRECT: (name: string) => `${name} data looks incorrect. Please recount.`,
    STAT_TRUTH_NO_SALES: (name: string) => `No ${name} sold this period.`,
    STAT_TRUTH_SALES: (name: string, sold: number, profit: string) => `You sold ${sold} ${name}, making ${profit} profit.`,
    STAT_TRUTH_SALES_ABOUT: (name: string, sold: number, profit: string) => `You sold about ${sold} ${name}, making about ${profit} profit.`,
    STAT_TRUTH_SALES_ROUGH: (name: string, sold: number, profit: string) => `You roughly sold ${sold} ${name}, making roughly ${profit} profit.`,
    STAT_TRUTH_LOSING: (name: string, sold: number, loss: string) => `You sold ${sold} ${name}, losing ${loss}.`,
    STAT_CREDIT_PAID_UP: (name: string) => `${name} is all paid up.`,
    STAT_CREDIT_CHANGE: (name: string, amount: string) => `You owe ${name} ${amount} in change.`,
    STAT_CREDIT_OWES: (name: string, amount: string) => `${name} owes ${amount}.`,
    STAT_CREDIT_OVERDUE_TODAY: (name: string, amount: string) => `${name} owes ${amount}, and said they would pay today.`,
    STAT_CREDIT_OVERDUE: (name: string, amount: string, days: number) => `${name} owes ${amount}, and was meant to pay ${days} ${days === 1 ? 'day' : 'days'} ago.`,
    STAT_CREDIT_STALE: (name: string, amount: string, days: number) => `${name} owes ${amount}, and has not paid anything for ${days} days.`,
    STAT_OUTSTANDING: (amount: string, people: number) => `${amount} is owed to you by ${people === 1 ? '1 person' : `${people} people`}.`,
    STAT_NET_NO_EXPENSES: (gross: string) => `You made ${gross} from sales. No expenses recorded yet.`,
    STAT_NET_LOSS: (loss: string) => `Your costs were ${loss} more than you made from sales.`,
    STAT_NET_BREAK_EVEN: "You broke even: sales covered your costs exactly.",
    STAT_NET_PROFIT: (gross: string, expenses: string, net: string) => `You made ${gross} from sales, paid ${expenses} in costs, and kept ${net}.`,
    STAT_SALES_EMPTY: (month: string) => `Nothing recorded for ${month}.`,
    STAT_SALES_DAYS: (month: string, sales: string, profit: string, days: number) => `${month}: you took ${sales} and kept about ${profit} (${days} ${days === 1 ? 'day' : 'days'} recorded).`,
    STAT_SALES_MONTH: (month: string, sales: string, profit: string) => `${month}: you took ${sales} and kept about ${profit} (one month total).`,
    STAT_SALES_BOOK: (profit: string, months: number) => `${profit} profit across ${months === 1 ? '1 month' : `${months} months`}.`,
    SHARE: "Share",
    SHARE_REMINDER: "Remind",
    SHARE_RECEIPT: "Save & share receipt",
    SHARE_CREDIT_REMINDER: (name: string, amount: string, days: number | null) => `Hi ${name}. Your ShopTrack balance is ${amount}.${days != null ? ` The book was last updated ${days} ${days === 1 ? 'day' : 'days'} ago.` : ''} Please let us know when you can pay.`,
    SHARE_CREDIT_OVERDUE: (name: string, amount: string, days: number) => `Hi ${name}. Your ShopTrack balance is ${amount}, and the promised payment is ${days} ${days === 1 ? 'day' : 'days'} late. Please let us know when you can pay.`,
    SHARE_PAYMENT_RECEIPT: (name: string, amount: string, method: string, remaining: string, when: string) => `ShopTrack receipt\n${name} paid ${amount} by ${method} on ${when}.\nBalance left: ${remaining}.`,
    SHARE_COUNT_SUMMARY: (units: number, profit: string, counts: number) => `ShopTrack count\n${units} units sold. Estimated profit: ${profit}.\nBased on ${counts} saved ${counts === 1 ? 'count' : 'counts'}.`,
    SHARE_CASHUP_VERDICT: (verdict: 'balanced' | 'short' | 'over') => ({ balanced: 'Balanced', short: 'Short', over: 'Over' })[verdict],
    SHARE_CASHUP_SUMMARY: (verdict: string, counted: string, expected: string, gap: string, digital: string) => `ShopTrack cash-up: ${verdict}\nCash counted: ${counted}\nExpected cash: ${expected}\nGap: ${gap}\nDigital takings: ${digital}`,
    SHARE_SIGNOFF: (shop: string) => `— ${shop}`,
    READ_ALOUD: "Read aloud",
    STOP_READING: "Stop reading",
    SPEECH_LOCALE: "en-ZA",
    VOICE_ENTER: "Say number",
    VOICE_LISTENING: "Listening...",
    VOICE_NOT_A_NUMBER: "I did not hear one number. Please try again or type it.",
    VOICE_PERMISSION: "Microphone permission is needed to say a count.",
    VOICE_UNAVAILABLE_HINT: "Offline voice entry is not available right now. You can still type the number.",
    BARCODE_OPTIONAL: "Barcode (optional)",
    BARCODE_HINT: "Use this only to find the product during a count or delivery.",
    SCAN_BARCODE: "Scan barcode",
    CLOSE_SCANNER: "Close camera",
    CAMERA_PERMISSION: "Camera permission is needed to scan a barcode.",
    BARCODE_NOT_FOUND: "No active product has this barcode. Add it in Product Details first.",
    BARCODE_LOOKING_UP: "Looking up the product name...",
    SHOW_ALL_PRODUCTS: "Show all products",
    SETTINGS: "Settings",
    SHOP_PROFILE: "Your shop",
    SHOP_PROFILE_HINT: "The shop name goes on messages you share: reminders, receipts, and orders.",
    SHOP_NAME_PLACEHOLDER: "Shop name",
    SHOP_PHONE_PLACEHOLDER: "Shop phone (optional)",
    SHOP_PROFILE_SAVE: "Save shop details",
    SHOP_PROFILE_SAVED: "Shop details saved",
    APP_TAGLINE_NAMED: "ShopTrack · Know your profit",
    COUNTRY_PACK: "Country pack",
    COUNTRY_PACK_HINT: "Sets the local currency and mobile-money name. You can change either later.",
    CURRENCY_LABEL: "Currency",
    LANGUAGE_REVIEW_PENDING: "Native-speaker review pending",
    CLOUD_BACKUP_TITLE: "Encrypted cloud safety net",
    CLOUD_BACKUP_HINT: "Optional. Your books are encrypted on this phone before upload. There is no account.",
    CLOUD_BACKEND_MISSING: "Cloud storage is not configured in this build. Local and shared backups still work.",
    CLOUD_CREATE_PHRASE: "Create recovery phrase",
    CLOUD_PHRASE_WARNING: "Write these 12 words on paper. Anyone with them can restore your books. ShopTrack cannot recover them for you.",
    CLOUD_WROTE_PHRASE: "I wrote it down",
    CLOUD_PHRASE_SAVED: "Recovery phrase saved on this phone",
    CLOUD_UPLOAD: "Upload encrypted backup",
    CLOUD_UPLOAD_DONE: "Encrypted backup uploaded",
    CLOUD_AUTO_BACKUP_HINT: "Plus can upload the newest encrypted backup automatically after the daily private copy. Offline failures stay queued.",
    CLOUD_PLUS_REQUIRED: "Automatic backup needs ShopTrack Plus. Manual encrypted upload and every local backup stay available.",
    CLOUD_AUTO_BACKUP_ENABLE: "Turn on automatic backup",
    CLOUD_AUTO_BACKUP_DISABLE: "Turn off automatic backup",
    CLOUD_RESTORE_PHRASE: "Enter the 12-word recovery phrase",
    CLOUD_DOWNLOAD: "Download and restore",
    CLOUD_BAD_PHRASE: "That is not a valid 12-word recovery phrase.",
    CLOUD_ERROR: "Encrypted cloud backup failed. Your data on this phone was not changed.",
    CLOUD_RESTORE_READY: "Encrypted backup found",
    CLOUD_RESTORE_READY_HINT: "View it without changing this phone, or restore it and replace the data currently here.",
    CLOUD_RESTORE_DONE: "Encrypted backup restored",
    RESTORE_SAFETY_SNAPSHOT_HINT: "Before restoring, ShopTrack saves a private copy of the data currently on this phone.",
    RESTORE_UNDO_ACTION: "Undo restore",
    RESTORE_UNDO_DONE: "The previous data from this phone has been restored",
    CLOUD_VIEW_ACTION: "View only",
    CLOUD_VIEWER_TITLE: "My Shop",
    CLOUD_VIEWER_READ_ONLY: "Read only. Nothing here changes this phone or the shop phone.",
    CLOUD_VIEWER_BACKUP_AT: (when: string) => `Backup from ${when}`,
    CLOUD_VIEWER_OVERVIEW: "Overview",
    CLOUD_VIEWER_STOCK_VALUE: "Stock selling value",
    CLOUD_VIEWER_TOTAL_EXPENSES: "Total expenses",
    CLOUD_VIEWER_TOTAL_TAKINGS: "Sales written down",
    CLOUD_VIEWER_ESTIMATED_PROFIT: "Estimated sales-book profit",
    CLOUD_VIEWER_COUNT_RESULT: (units: number, revenue: string) => `${units} units sold · ${revenue} taken`,
    CLOUD_VIEWER_NO_ROWS: "Nothing recorded here.",
    OWNER_LOCK: "Owner lock",
    OWNER_LOCK_HINT: "Optional. A 4-digit PIN hides your profit, sales book, expenses, and reports from whoever holds the phone. Counting, stock-in, today's sales, cash-up, and the credit book stay open.",
    OWNER_LOCK_ON: "Owner lock is on",
    OWNER_PIN_PLACEHOLDER: "New 4-digit owner PIN",
    OWNER_PIN_CURRENT: "Current owner PIN",
    OWNER_LOCK_ENABLE: "Turn on owner lock",
    OWNER_LOCK_CHANGE: "Change owner PIN",
    OWNER_LOCK_DISABLE: "Turn off owner lock",
    OWNER_LOCK_CHANGED: "Owner lock updated",
    OWNER_WRONG_PIN: "That is not the owner PIN.",
    OWNER_ONLY_TITLE: "Owner only",
    OWNER_ONLY_HINT: "Enter the owner PIN to see this.",
    OWNER_UNLOCK: "Unlock",
    OWNER_LOCKED_CARD: "🔒 Owner only",
    OWNER_LOCKED_CARD_HINT: "Profit and money totals are hidden. Tap to enter the owner PIN.",
    STAFF_MODE: "Staff mode",
    STAFF_MODE_HINT: "Optional. A 4-digit PIN attributes counts and cash-ups on this shared phone; it is not a security lock.",
    STAFF_NAME: "Staff name",
    STAFF_PIN: "4-digit PIN",
    STAFF_ADD: "Add staff member",
    STAFF_REMOVE: "Remove",
    STAFF_WHO: "Who is doing this?",
    STAFF_ENTER_PIN: "Enter your 4-digit PIN",
    STAFF_CONFIRM: "Confirm person",
    STAFF_WRONG_PIN: "That PIN does not match an active staff member.",
    STAFF_REQUIRED: "Choose the staff member doing this before saving.",
    STAFF_RECORDED_AS: (name: string) => `Recording as ${name}`,
    HEALTH_TITLE: "Business health report",
    HEALTH_HINT: "A factual 90-day summary you choose to share. ShopTrack does not lend, score you, or send this anywhere by itself.",
    HEALTH_PERIOD: (days: number) => `Reporting period: last ${days} days`,
    HEALTH_COUNTS: (sessions: number, days: number) => `Stock counting: ${sessions} ${sessions === 1 ? 'session' : 'sessions'} across ${days} separate ${days === 1 ? 'day' : 'days'}`,
    HEALTH_MARGIN: (margin: string, priced: number, total: number) => `Known shelf margin: ${margin}% average (${priced} of ${total} ${total === 1 ? 'product has' : 'products have'} both prices)`,
    HEALTH_MARGIN_UNKNOWN: "Known shelf margin: not enough priced products yet",
    HEALTH_CREDIT: (given: string, repaid: string, rate: string) => `Credit book: ${given} given, ${repaid} repaid (${rate})`,
    HEALTH_CASHUPS: (total: number, balanced: number, rate: string) => `Cash-up discipline: ${balanced} of ${total} balanced within tolerance (${rate})`,
    HEALTH_SHARE: "Share this report",
    HEALTH_SHARE_HEADER: "ShopTrack business health report",
    NOT_AVAILABLE: "not available",
    REORDER_SHARE: "Share reorder sheet",
    REORDER_LINE: (name: string, current: number, suggested: number, unit: string) => `${name}: ${current} ${unit} left · order about ${suggested}`,
    REORDER_HEADER: "ShopTrack reorder request",
    PARTNER_TITLE: "Partner attribution",
    PARTNER_HINT: "Optional referral code. Activation facts stay on this phone until you explicitly share the export.",
    PARTNER_CODE: "Referral code",
    PARTNER_SAVE: "Save referral code",
    PARTNER_BAD_CODE: "Use 2–32 letters, numbers, hyphens, or underscores.",
    PARTNER_EXPORT: "Share activation export",
    PARTNER_EXPORT_HINT: "This export contains activation dates and the referral code only—no shop names, products, customers, or amounts.",
    SHOW_MORE: "Show 50 more",
    PHOTO_LABEL: "Photo (optional)",
    PHOTO_ADD: "Add photo",
    PHOTO_TAKE: "Take photo",
    PHOTO_CHOOSE: "Choose from phone",
    PHOTO_CHANGE: "Change photo",
    PHOTO_REMOVE: "Remove photo",
    PHOTO_PERMISSION_TITLE: "Photo permission needed",
    PHOTO_CAMERA_PERMISSION: "Allow camera access to take this photo.",
    PHOTO_LIBRARY_PERMISSION: "Allow photo access to choose this photo.",
    PHOTO_SAVE_ERROR: "The photo was not saved. Please try again.",
    PRODUCT_PHOTO: "Product photo",
    CUSTOMER_PHOTO: "Customer photo or ID",
    CUSTOMER_PHOTO_PRIVATE_HINT: "Customer/ID photos stay on this phone and in encrypted backups. Plain shared backup files leave them out.",
    RECEIPT_PHOTO: "Receipt photo",
    PHOTO_PRIVATE_HINT: "Stays on this phone and in backups you create.",
};
